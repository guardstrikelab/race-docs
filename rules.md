...
训练场景包含30个原子场景，用于提供给参赛选手进行自动驾驶算法的开发和调试。所有训练场景都可以查看，也可以作为种子场景使用场景泛化功能衍生出更多的场景来满足自动驾驶算法开发所需的数据采集需求和场景多样性需求。
...
参赛选手在一个月内或一天内的提交机会是有限的（一个月提交次数不超过 `30` 次，一天内不超过 `2` 次）。
